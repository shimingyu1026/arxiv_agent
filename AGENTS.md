<!-- From: /Users/smy/Proj/tmp/nothing/AGENTS.md -->
...
- `RESEARCH_AREAS` — 研究方向关键词，支持环境变量注入（默认：`AI芯片、机器人芯片、具身智能、Neuro-Symbolic AI`）
...
- **触发方式**: 
  - `schedule: cron("0 22 * * *")` — 每天 UTC 22:00 自动运行（北京时间 06:00），预留 GitHub runner 排队延迟
...
- **即时提交**: 完成一个需求或任务后，应立即执行 `git commit` 并 `git push`，确保代码及时同步到远程仓库，避免工作丢失。
