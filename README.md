...
- 📰 每日自动抓取 `cs.AR` 与 `cs.RO` 最新论文（北京时间每天早上 6:00 推送）
...
工作流默认每天早上 **北京时间 06:00** 自动运行。如果某天没有新论文，你会收到一封"今日暂无新论文"的提示邮件。
...
论文过滤窗口默认会按 GitHub Actions 的 UTC 22:00 运行时间自动选择：平日抓取最近 36 小时，UTC 周一抓取最近 96 小时用于覆盖周末堆积的 arXiv 提交。如需手动覆盖，可配置环境变量 `PAPER_LOOKBACK_HOURS`。
